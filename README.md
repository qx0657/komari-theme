...
- **月均和剩余价值。** 首页总览有一张卡汇总全部资源的剩余价值，副行是月均。VPS 节点和额外账单（域名、机场、ISP）算在一起，按人民币折算。`/assets` 里还有年化总支出、月均支出和收购溢价。
- **服务器卡片。** 标题旁可以复制 IP，地址悬停才显示。V4 后面是 IP 属性标签，比如原生、广播、家宽、IDC，按机器名或 UUID 在主题管理里配。卡片还可以看今日流量、费用到期和在线时间，视图有大卡、小卡、迷你卡和列表。
- **首页。** 总览下面是 ISP 余量、域名和机场；没有对应条目就不显示。可以按分组、地区筛选，也可以改排序。总览可以收成一行。
- **外观。** 深色、浅色、跟随系统、像素农场。农场里可以给节点招牌指定颜色。背景图和卡片透明度在主题管理里调。

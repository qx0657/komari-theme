...
import { NodeCopyIpPopover } from "./NodeCopyIpPopover";
import { NodeHideButton } from "./NodeHideButton";
...
function MiniHeader({
  ...
  return (
    <header className="mini-node-header">
      ...
      <NodeHideButton uuid={node.uuid} name={node.name} size={13} />
      <NodeCopyIpPopover ipv4={node.ipv4} ipv6={node.ipv6} size={13} />
...
function MiniChips({
  ...
  if (!renewalPrice && tags.length === 0 && !ipv4 && !ipv6 && ipBadges.length === 0 && !offlineDuration) return null;
  ...
  return (
    <div className="mini-node-chip-row">
      ...
      <IpStackBadges ipv4={ipv4} ipv6={ipv6} extra={ipBadges} />
      {tags.length > 0 && (

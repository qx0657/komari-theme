...
import { overlayLiveExtraAssets } from "@/utils/extraAssets";
import { useHiddenNodeUuids } from "@/hooks/useVisibleNodes";
import { showAllHomeCards, useHomeCardHidden } from "@/hooks/useHomeCardHidden";
...
function HomeOverviewCards({
  ...
  costSummary: { remainingCny: number; monthlyCny: number } | null;
  costLoading: boolean;
  dense: boolean;
  ...
  return (
    <section className={`home-overview${dense ? " is-dense" : ""}`} aria-label="首页总览">
      ...
      <article className="overview-card" data-metric="asset">
        ...
        <div className="overview-card-footer">
          <div className="overview-card-caption-stack">
            <p className="home-asset-monthly">
              {costSummary
                ? `月均 ${formatCnyMoney(costSummary.monthlyCny)}`
                : costLoading
                  ? "月均计算中"
                  : "月均 —"}
            </p>
            <p className="overview-card-caption">剩余价值 · 实时汇率</p>
          </div>
...
// 与分组栏是两条独立筛选,可叠加(先分组、后地区)。右侧「全部显示」清掉地区筛选,
// 并把本机隐藏的卡片一起放回来。
function RegionTabs({
  regions,
  selectedRegion,
  ...
  const canShowAll = hiddenCount > 0 || selectedRegion !== HOME_ALL_REGION;
  const showAllLabel =
    hiddenCount > 0 ? `全部显示，恢复 ${hiddenCount} 台已隐藏服务器` : "全部显示";
  return (
    <section className="home-region-bar" aria-label="地区筛选">
      ...
      <button
        type="button"
        className="home-region-show-all"
        onClick={onShowAll}
        disabled={!canShowAll}
      ...
      >
        全部显示
        {hiddenCount > 0 && <span className="home-region-chip-count">{hiddenCount}</span>}
      </button>
    </section>
  );
}
...
export function NodeGrid() {
  ...
  const cardHidden = useHomeCardHidden();
  const showEveryCard = useCallback(() => {
    showAllHomeCards();
  ...
  const extraAssets = useMemo(
    () => overlayLiveExtraAssets(themeSettings.extraAssets, ispQuery.data),
    [ispQuery.data, themeSettings.extraAssets],
  );
  ...
  const extraDetails = useMemo(
    () => (costSummary?.details ?? []).filter((detail) => detail.kind !== "node"),
    [costSummary],
  );
  ...
  // 地区计数只算还在卡片上的节点。隐藏前的地区数用来决定栏是否保留,
  // 避免藏掉某个地区的最后一台后，连「全部显示」一起消失。
  const regionCountBeforeHide = useMemo(
    () => getHomeRegionOptions(groupFilteredNodes).length,
    [groupFilteredNodes],
  );
  const shownInGroup = useMemo(
    () => groupFilteredNodes.filter((node) => !cardHidden.hidden.has(node.uuid)),
    [cardHidden.hidden, groupFilteredNodes],
  );
  const hiddenInView = groupFilteredNodes.length - shownInGroup.length;
  const regionOptions = useMemo(
    () => getHomeRegionOptions(shownInGroup),
    [shownInGroup],
  );
  const filteredNodes = useMemo(
    () =>
      selectedRegion === HOME_ALL_REGION
        ? shownInGroup
        : shownInGroup.filter((node) => getDisplayRegionCode(node.region) === selectedRegion),
    [selectedRegion, shownInGroup],
  );
  ...
  // 地区栏:只有一个地区时筛选无意义,>1 才显示。有本机隐藏时也保留这条,
  // 好让右侧「全部显示」还在。
  const regionFilterEnabled =
    themeSettings.isReady && themeSettings.showRegionBar && regionCountBeforeHide > 1;
  const showRegionBar = regionFilterEnabled || hiddenInView > 0;
    ...
    <>
      ...
      {showHomeOverview && (
        <>
          ...
          <HomeResourceStrip extraDetails={extraDetails} extraAssets={extraAssets} />
        </>
      )}
    </>
  );
  ...
  return (
    <>
      ...
      {showRegionBar && (
        <RegionTabs
          regions={regionFilterEnabled ? regionOptions : []}
          selectedRegion={selectedRegion}
          onSelectRegion={setSelectedRegion}
          hiddenCount={hiddenInView}
          onShowAll={showEveryCard}
        />
      )}
      {orderedUuids.length === 0 && hiddenInView > 0 ? (
        <p className="home-cards-empty">这些服务器已从首页隐藏，点「全部显示」恢复。</p>
      ) : isList ? (

...
export function installDevMockApi() {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.origin === window.location.origin && url.pathname === "/isp/api/admin/credentials") {
      if (init?.method === "POST") {
        return json({
          ok: true,
          api_token_set: true,
          ...
          session_token_tail: "jwt4",
          session_expires_at: "2026-11-04T00:00:00+00:00",
        });
      }
      return json({
        api_token_set: true,
    ...
    if (url.origin === window.location.origin && url.pathname === "/isp/api/usage") {
      const days = Number(url.searchParams.get("days") || "7");
      const series: Record<string, number> = {};
      const today = new Date();
      for (let i = Math.max(1, Math.min(days, 30)) - 1; i >= 0; i--) {
        const d = new Date(today);
        d.setDate(today.getDate() - i);
        const key = d.toISOString().slice(0, 10);
        series[key] = Math.round((40 + (i % 5) * 17 + i * 3) * 10) / 10;
      }
      return json({
        ...
        active: {
          ...
          renew_plan: "30 Days",
          renew_pay: "balance",
          renews_at: "2026-09-25",
          proxies: [{ ip: "205.179.246.63" }],
        },
        orders: [
          {
            id: 73297593,
        ...
        usage: { ok: true, series_mb: series, remaining: { gb_balance: 77.52, gb_limit: 100, gb_used: 22.48 } },
      });
    }
    ...
    if (url.pathname === "/api/public") {
      ...
      return json({
        ...
        theme_settings: savedThemeSettings[theme] ?? {
          ...
          ipProfiles: [
            { identities: ["Tokyo Edge"], badges: [{ label: "原生" }, { label: "IDC机房" }] },
            { identities: ["Singapore API"], badges: [{ label: "原生" }, { label: "IDC机房" }, { label: "NAT" }] },
            { identities: ["Frankfurt DB"], badges: [{ label: "广播" }, { label: "IDC机房" }, { label: "NAT" }] },
            { identities: ["New York Worker"], badges: [{ label: "原生" }, { label: "IDC机房" }, { label: "CN2GIA" }] },
            { identities: ["Hong Kong Cache"], badges: [{ label: "原生" }, { label: "家宽" }] },
            { identities: ["Sydney Backup"], badges: [{ label: "广播" }, { label: "IDC机房" }] },
          ],
        },
      });
    }

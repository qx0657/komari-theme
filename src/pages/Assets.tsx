...
import { EXTRA_ASSET_KIND_LABEL, overlayLiveExtraAssets } from "@/utils/extraAssets";
import { useIspUsage } from "@/hooks/useIspUsage";
import { formatBillingCycle, formatPostedPrice } from "@/utils/billing";
...
function assetPriceLabel(detail: AssetDetail) {
  if (detail.kind !== "node") {
    return (
      formatPostedPrice(detail.sourcePrice, detail.sourceCurrency, detail.billingCycleDays) ??
      detail.note ??
      "—"
    );
  }
  if (!detail.counted) return null;
  return `${formatCnyMoney(detail.priceCny)}/${formatBillingCycle(detail.billingCycleDays)}`;
}
  ...
  const summary = useMemo(
    () =>
      rateQuery.data
        ...
        : null,
    [extraAssets, nodes, now, themeSettings.costIgnoredNodes, themeSettings.costPremiums, rateQuery.data],
  );
      ...
      {nodes.length === 0 ? (
        ...
        <>
          ...
          <div className="assets-section-head">
            ...
            <span className="assets-count">{detailRows.length} 项</span>
              ...
              {!isMobileLayout ? (
                      ...
                      return (
                        ...
                        <td>
                          {detail.kind === "node" ? (
                            ...
                            <span className="assets-node-link" title={detail.name}>
                              <span className="home-bill-kind">
                                {EXTRA_ASSET_KIND_LABEL[detail.kind] ?? detail.kind}
                              </span>
                              <span>{detail.name}</span>
                            </span>
                          )}
                ...
                {detailRows.map((detail) => {
                  ...
                  const renewalLabel = reminder
                    ...
                    : undefined;
                  const priceLabel =
                    assetPriceLabel(detail) ?? detail.note ?? "—";
                  return (
                    <div
                      key={detail.uuid}
                    ...
                    >
                      <div className="cost-summary-detail-head">
                        {detail.kind === "node" ? (
                          ...
                          <span className="cost-summary-detail-name">
                            <span className="home-bill-kind">
                              {EXTRA_ASSET_KIND_LABEL[detail.kind] ?? detail.kind}
                            </span>
                            <span className="cost-summary-detail-title">{detail.name}</span>

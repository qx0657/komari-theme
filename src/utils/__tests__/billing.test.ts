...
import { formatBillingCycle, formatPostedPrice, formatRenewalPrice } from "@/utils/billing";
...
describe("formatPostedPrice", () => {
  it("keeps billed currency and cycle, including Turkish lira years", () => {
    expect(formatPostedPrice(120, "TRY", 365)).toBe("₺120/年");
    expect(formatPostedPrice(100, "CNY", 365)).toBe("¥100/年");
    expect(formatPostedPrice(199.5, "CNY", 730)).toBe("¥199.50/2年");
    expect(formatPostedPrice(10, "USD", 30)).toBe("$10/月");
    expect(formatPostedPrice(12.5, "USD", 365)).toBe("$12.50/年");
  });
});

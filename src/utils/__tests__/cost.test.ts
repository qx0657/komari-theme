...
describe("cost helpers", () => {
  ...
  it("adds extra cloud assets into remaining and monthly totals", () => {
    const summary = calculateCostSummary(
      [node({ price: 10, currency: "USD", billing_cycle: 30, expired_at: inDays(30) })],
      [],
      { USD: 1, CNY: 7, TRY: 35 },
      {},
      Date.now(),
    ...
    expect(summary.details.filter((item) => item.kind !== "node")).toHaveLength(2);
    expect(summary.monthlyCny).toBeGreaterThan(70);
    expect(summary.remainingCny).toBeGreaterThan(70);
  });
  ...
  it("ignores malformed or future-dated exchange-rate caches", async () => {
    ...
    const fetchMock = vi.fn().mockImplementation(async () =>
      new Response(JSON.stringify({ rates: { CNY: 7, TRY: 34 } }), {
        status: 200,

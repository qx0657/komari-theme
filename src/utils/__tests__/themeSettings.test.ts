...
describe("normalizeThemeSettings", () => {
  ...
  it("normalizes homepage multi-ping tasks while preserving an enabled draft for repair", () => {
    ...
    expect(normalizeThemeSettings({}).extraAssets).toEqual([]);
    expect(normalizeThemeSettings({}).enableIspStrip).toBe(true);
    expect(normalizeThemeSettings({}).enableAirportStrip).toBe(true);
    expect(normalizeThemeSettings({}).airportUsageUrl).toBe("");
    expect(normalizeThemeSettings({ airportUsageUrl: " /usage/airports.json " }).airportUsageUrl).toBe(
      "/usage/airports.json",
    );
    expect(
      normalizeThemeSettings({ airportUsageUrl: "https://example.com/sub?token=1" }).airportUsageUrl,
    ).toBe("");
    expect(normalizeThemeSettings({ airportUsageUrl: "//example.com/usage" }).airportUsageUrl).toBe(
      "",
    );
    expect(normalizeThemeSettings({ airportUsageUrl: "/usage/../secret" }).airportUsageUrl).toBe("");
    expect(normalizeThemeSettings({ enableIspStrip: false }).enableIspStrip).toBe(false);
    expect(
      normalizeThemeSettings({
  ...
  it("leaves homepage ping bindings empty until the operator saves them", () => {
    expect(normalizeThemeSettings({}).homepagePingBindings).toEqual({});
    expect(normalizeThemeSettings({}).homepageMultiPingTaskIds).toEqual([]);
  ...
  it("defaults home sort to weight ascending and falls back to a field's natural direction", () => {
    ...
    expect(base.showGroupTabs).toBe(false);
  ...
  it("defaults IP attribute profiles to an empty shareable list", () => {
    expect(normalizeThemeSettings({}).ipProfiles).toEqual([]);
    expect(normalizeThemeSettings({ ipProfiles: [] }).ipProfiles).toEqual([]);
    expect(
      normalizeThemeSettings({
        ipProfiles: [{ identities: ["node-a"], badges: ["原生"] }],
      }).ipProfiles[0]?.identities,
    ).toEqual(["node-a"]);
  });
});

...
const POSTED_CURRENCY_SYMBOL: Record<string, string> = {
  CNY: "¥",
  元: "¥",
  "人民币": "¥",
  "¥": "¥",
...
export function formatPostedPrice(
  price: number,
  ...
  if (upper === "TRY" || upper === "TL" || raw === "₺" || raw.includes("里拉")) {
    return `₺${formatPriceNumber(price)}/${cycle}`;
  }
  const symbol = POSTED_CURRENCY_SYMBOL[raw] ?? POSTED_CURRENCY_SYMBOL[upper] ?? raw ?? "¥";
  return `${symbol}${formatPriceNumber(price)}/${cycle}`;
}

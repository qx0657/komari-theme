...
import type { ExtraAsset, ExtraAssetKind } from "@/utils/extraAssets";
...
const TRY_RATE_URL = "https://open.er-api.com/v6/latest/USD";

async function ensureTryRate(
  rates: Record<string, number>,
  signal?: AbortSignal,
): Promise<Record<string, number>> {
  if (rates.TRY && rates.TRY > 0) return rates;
  try {
    ...
    const tryRate = Number(payload.rates?.TRY);
    if (Number.isFinite(tryRate) && tryRate > 0) {
      return { ...rates, TRY: tryRate };
    }
  } catch {
    // TRY 不是 ECB 货币；补源失败时域名行会记「汇率缺失」，不伪造牌价。
  }
  return rates;
}
...
export async function getExchangeRates(
  ...
  try {
    ...
    const rates = await ensureTryRate(parsed.rates, options.signal);
...
export function calculateCostSummary(
  ...
  for (const node of nodes as CostNode[]) {
    ...
    const baseDetail = {
      ...
      sourcePrice: Number(node.price) || 0,
      sourceCurrency: String(node.currency || "").trim(),
    };
  ...
  extraAssets.forEach((asset, index) => {
    const cycleDays = billingCycleDays(asset.billingCycle);
    const extraBase = {
      uuid: asset.id,
      name: asset.name,
      region: asset.region || "",
      ...
      premiumRemainingCny: 0,
      billingCycleDays: cycleDays,
      sourcePrice: Number(asset.price) || 0,
      sourceCurrency: String(asset.currency || "").trim(),
    };
    const price = Number(asset.price) || 0;
    if (price <= 0) {
      details.push({ ...extraBase, counted: false, note: "免费" });
      return;
    }
    const converted = convertToCny(price, asset.currency, rates);
    if (converted == null || !Number.isFinite(converted)) {
      details.push({ ...extraBase, counted: false, note: "汇率缺失" });
      return;
    }
    const months = cycleMonths(cycleDays);
    const monthly = months > 0 ? converted / months : 0;
    const remaining = remainingCycleValue(converted, cycleDays, asset.expiredAt, now);
    totalCny += monthly * 12;
    monthlyCny += monthly;
    remainingCny += remaining;
    details.push({
